...
const aSTModified = (ast) => {
    ...
    const type = ast.type;
    const keys = type === 'select' ? keys_select : type === 'insert' ? keys_insert : type === 'update' ? keys_update : keys_delete;
    for (let k of keys) {
        if (!(k in ast))
            return true;
    }
...
const checkUnmodifiedASTs = (obj) => {
    if (Array.isArray(obj)) {
        for (let o of obj) {
            if (!aSTModified(o[Object.keys(o)[0]].ast))
                return o;
        }
    }
    else if (!aSTModified(obj[Object.keys(obj)[0]].ast))
        return obj;
    return null;
};
const generateSelectColumnsRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'SELECT',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
        parent_statement: parent_query_statement === null || parent_query_statement === void 0 ? void 0 : parent_query_statement.toUpperCase(),
        recommendation: [],
    };
    let sub_message = ' of the nested query in the ' + (parent_query_statement === null || parent_query_statement === void 0 ? void 0 : parent_query_statement.toUpperCase()) + ' statement';
    let diff_message0 = diff_type === 'missing' ? 'ALL' : 'ONLY';
    if (!('expr' in obj && 'type' in obj.expr)) {
        let message = 'An unknown object has been found in the SELECT statement' +
            (sub ? sub_message : '') +
        ...
        let o = obj.expr;
        let message0 = 'Make sure the SELECT statement' +
            (sub ? sub_message : '') +
            ' includes ' +
            diff_message0 +
        ...
        let message1 = 'Aggregate function ' +
            o.name +
            '(...)' +
            (obj.as ? ' AS ' + obj.as : '') +
            ' in the SELECT statement' +
        ...
        let message1 = "Binary expression '... " +
            o.operator +
            " ...'" +
            (obj.as ? ' AS ' + obj.as : '') +
            ' in the SELECT statement' +
        ...
        let message1 = "Window function '" +
            ...
            diff_type +
            ' or contains invalid values.';
        let message2 = 'Unfortunately, the capabilities of this recommendation system are limited. Content of a window function ' +
...
const generateSelectDistinctRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'DISTINCT',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateSelectFromRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'FROM',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
    ...
    if (Object.keys(obj).length === 3 && 'db' in obj && 'table' in obj && 'as' in obj) {
        ...
        let message = 'An unknown object has been found in the FROM/JOIN statement' +
            (sub ? sub_message : '') +
            '. Please report this as bug, stating the chapter name, exercise number and your query. Thank you! Your help is much appreciated.';
        rec.recommendation.push(message, message, message);
    }
...
const generateSelectWhereRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'WHERE',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
    ...
    if ('type' in obj && 'operator' in obj && 'left' in obj && 'right' in obj) {
        ...
        let message = 'An unknown object has been found in the WHERE statement' +
            (sub ? sub_message : '') +
            '. Please report this as bug, stating the chapter name, exercise number and your query. Thank you! Your help is much appreciated.';
        rec.recommendation.push(message, message, message);
    }
...
const generateSelectGroupByRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'GROUPBY',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
    ...
    let message0 = 'Make sure the GROUP BY statement' +
        (sub ? sub_message : '') +
        ' includes ' +
        diff_message +
        ' the required columns or expressions.';
    let message1 = 'Make sure the columns or expressions in the GROUP BY statement' +
        (sub ? sub_message : '') +
    ...
    if ('type' in obj && 'table' in obj && 'column' in obj) {
        ...
        if (o.type === 'function' && o.args.value[0].type === 'column_ref' && o.args.value[1].type === 'column_ref') {
            let col1 = o.args.value[0].table === null ? o.args.value[0].column : o.args.value[0].table + '.' + o.args.value[0].column;
            let col2 = o.args.value[1].table === null ? o.args.value[1].column : o.args.value[1].table + '.' + o.args.value[1].column;
            message2 =
                o.name +
                    '(' +
                    col1 +
        ...
        let message = 'An unknown object has been found in the GROUP BY statement' +
            (sub ? sub_message : '') +
            '. Please report this as bug, stating the chapter name, exercise number and your query. Thank you! Your help is much appreciated.';
        rec.recommendation.push(message, message, message);
    }
...
const generateSelectHavingRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'HAVING',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
    ...
    if ('type' in obj && 'operator' in obj && 'left' in obj && 'right' in obj && obj.type === 'binary_expr') {
        ...
        let left;
        let right;
        if (o.left.type === 'column_ref')
            left = o.left.table === null ? o.left.column : o.left.table + '.' + o.left.column;
        ...
        let message = 'An unknown object has been found in the HAVING statement' +
            (sub ? sub_message : '') +
            '. Please report this as bug, stating the chapter name, exercise number and your query. Thank you! Your help is much appreciated.';
        rec.recommendation.push(message, message, message);
    }
...
const generateSelectOrderByRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'ORDERBY',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
    ...
    if ('expr' in obj && 'type' in obj) {
        ...
        let message = 'An unknown object has been found in the ORDER BY statement' +
            (sub ? sub_message : '') +
            '. Please report this as bug, stating the chapter name, exercise number and your query. Thank you! Your help is much appreciated.';
        rec.recommendation.push(message, message, message);
    }
...
const generateSelectLimitRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'SELECT',
        statement: 'LIMIT',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateInsertTableRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'INSERT',
        statement: 'TABLE',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateInsertColumnsRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
...
const generateInsertValuesRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
...
const generateInsertSelectRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    // INSERT INTO table(columns) VALUES --> generujem odporucania pre VALUES
    // values : [ {}, {} , {}], kde {} reprezentuje 1 riadok VALUES (x,x,y,z)
    // {} obsahuje type a value: [{x},{x},{y},{z}]
    // ak vsak values : {}, potom sa jedna o SELECT. Ak niekde v SELECT je ast, potom o INSERT .. SELECT x,(SELECT ....),y, z
...
const generateUpdateTableRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'UPDATE',
        statement: 'TABLE',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateUpdateSetRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'UPDATE',
        statement: 'SET',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateUpdateWhereRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'UPDATE',
        statement: 'WHERE',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateDeleteFromRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'DELETE',
        statement: 'FROM',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const generateDeleteWhereRecommendations = (obj, diff_type, sub, parent_query_type, parent_query_statement) => {
    let rec = {
        query_type: 'DELETE',
        statement: 'WHERE',
        parent_query_type: parent_query_type === null || parent_query_type === void 0 ? void 0 : parent_query_type.toUpperCase(),
...
const recommender = (diff, diff_type, recs, sub, query_type, branch) => {
            ...
            if (key === 'distinct') {
                if (typeof value === 'object' && value !== null && 'type' in value) {
                    if (typeof value.type === 'string' && value.type !== null) {
                        recs.push(generateSelectDistinctRecommendations(value, diff_type, sub, sub ? query_type : undefined, branch));
                    }
                }
            }
            else if (key === 'limit') {
                ...
                if (Array.isArray(value) && value.length > 0) {
                    for (let [v_key, v_val] of Object.entries(value)) {
                        if (typeof v_val === 'object' && v_val !== null && 'ast' in v_val) {
                            const subAST = v_val['ast'];
                            recommender(subAST, diff_type, recs, true, 'select', branch ? branch : 'select');
                        }
                        else if (key === 'columns') {
                            recs.push(generateSelectColumnsRecommendations(v_val, diff_type, sub, sub ? query_type : undefined, branch));
                        }
                        else if (key === 'from') {
                            recs.push(generateSelectFromRecommendations(v_val, diff_type, sub, sub ? query_type : undefined, branch));
                        }
                        else if (key === 'where') {
                            recs.push(generateSelectWhereRecommendations(v_val, diff_type, sub, sub ? query_type : undefined, branch));
                        }
                        else if (key === 'groupby') {
                            recs.push(generateSelectGroupByRecommendations(v_val, diff_type, sub, sub ? query_type : undefined, branch));
                        }
                        else if (key === 'having') {
                            recs.push(generateSelectHavingRecommendations(v_val, diff_type, sub, sub ? query_type : undefined, branch));
                        }
                        else if (key === 'orderby') {
                            recs.push(generateSelectOrderByRecommendations(v_val, diff_type, sub, sub ? query_type : undefined, branch));
                        }
                    }
                }
                else if (typeof value === 'object' && value !== null) {
                    if ('ast' in value) {
                        const subAST = value['ast'];
                        recommender(subAST, diff_type, recs, true, 'select', branch ? branch : 'select');
                    }
                    else if (key === 'columns') {
                        recs.push(generateSelectColumnsRecommendations(value, diff_type, sub, sub ? query_type : undefined, branch));
                    }
                    else if (key === 'from') {
                        ...
                        recs.push(generateSelectGroupByRecommendations(value, diff_type, sub, sub ? query_type : undefined, branch));
                    }
                    else if (key === 'having') {
                        recs.push(generateSelectHavingRecommendations(value, diff_type, sub, sub ? query_type : undefined, branch));
                    }
                    else if (key === 'orderby') {
                        recs.push(generateSelectOrderByRecommendations(value, diff_type, sub, sub ? query_type : undefined, branch));
                    }
                }
            }
        }
...
const checkIfContainsSubAST = (obj, parent) => {
    let asts = [];
    for (let [key, value] of Object.entries(obj)) {
        if (typeof value === 'object' && value !== null) {
            if ('ast' in value) {
                ...
                if (parent === undefined) {
                    newObj[key] = value;
                    asts.push(Object.assign({}, newObj));
                }
                else {
                    newObj[parent] = value;
                    asts.push(Object.assign({}, newObj));
                ...
                for (let [k, v] of Object.entries(value)) {
                    if (typeof v === 'object' && v !== null) {
                        if ('ast' in v) {
                            ...
                            let ret = checkIfContainsSubAST(v, parent === undefined ? key : parent);
                            if (ret !== null) {
                ...
                let ret = checkIfContainsSubAST(value, parent === undefined ? key : parent);
                if (ret !== null) {
                    if (Array.isArray(ret))
                        asts.push(...ret);
                    else
...
const generateGeneralRecommendation = (queryType, branch, diff_type) => {
    if (queryType === 'select') {
        if (diff_type === 'missing') {
            let s = branch === 'columns'
            ...
            return ('Unfortunately, there is no known solution that uses nested query in the ' +
                (branch === 'columns' ? 'SELECT' : branch.toUpperCase()) +
        ...
        if (diff_type === 'missing') {
            return ('Apparently, it is needed to use nested query in the VALUES / SELECT statement. Here are some examples:\n' +
                'INSERT INTO ... VALUES (<values>, (SELECT ... nested query), <values>);\n' +
                'INSERT INTO ... SELECT <values>, (SELECT ... nested query), <values>;');
        }
        else if (diff_type === 'redundant') {
            return ('Unfortunately, there is no known solution that uses nested query in the VALUES / SELECT statement. Apparently, ' +
        ...
        if (diff_type === 'missing') {
            return ('Apparently, it is needed to use nested query in the WHERE statement. Here are some examples:\n' +
                'DELETE FROM ... WHERE <operand> <operator> (SELECT ... nested query);\n' +
                'DELETE FROM ... WHERE (SELECT ... nested query) <operator> <operand>;');
        }
        else if (diff_type === 'redundant') {
            return ('Unfortunately, there is no known solution that uses nested query in the WHERE statement. Apparently, ' +
                'this exercise can be solved without using nested query.');
        }
        else {
            return ('Unfortunately, there is no known solution that uses nested query in the same statement as you did. ' +
...
const createRecommendations = (queryType, missing, extras, cluster) => {
    let recs = [];
