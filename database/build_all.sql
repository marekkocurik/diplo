...
INSERT INTO users.solutions(exercise_id, query) VALUES
(69, 'with recursive recommenders(recommender) as ( select recommendedby from exercises.members where memid = 27 union all select mems.recommendedby from recommenders recs inner join exercises.members mems on mems.memid = recs.recommender ) select recs.recommender, mems.firstname, mems.surname from recommenders recs inner join exercises.members mems on recs.recommender = mems.memid order by memid desc;'),
(70, 'with recursive recommendeds(memid) as ( select memid from exercises.members where recommendedby = 1 union all select mems.memid from recommendeds recs inner join exercises.members mems on mems.recommendedby = recs.memid ) select recs.memid, mems.firstname, mems.surname from recommendeds recs inner join exercises.members mems on recs.memid = mems.memid order by memid;'),
(71, 'with recursive recommenders(recommender, member) as ( select recommendedby, memid from exercises.members union all select mems.recommendedby, recs.member from recommenders recs inner join exercises.members mems on mems.memid = recs.recommender ) select recs.member member, recs.recommender, mems.firstname, mems.surname from recommenders recs inner join exercises.members mems on recs.recommender = mems.memid where recs.member = 22 or recs.member = 12 order by recs.member asc, recs.recommender desc;');
