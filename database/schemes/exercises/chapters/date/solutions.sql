...
(59, 'select starttime, starttime + slots*(interval ''30 minutes'') endtime from exercises.bookings order by endtime desc, starttime desc limit 10;'),
(60, 'select date_trunc(''month'', starttime) as month, count(*) from exercises.bookings group by month order by month;'),
(61, 'select name, month, round((100*slots)/ cast( 25*(cast((month + interval ''1 month'') as date) - cast (month as date)) as numeric),1) as utilisation from ( select facs.name as name, date_trunc(''month'', starttime) as month, sum(slots) as slots from exercises.bookings bks inner join exercises.facilities facs on bks.facid = facs.facid group by facs.facid, month ) as inn order by name, month;');